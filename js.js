...
var greetingTime = 'Good night'
          ...
          sec.push({
            "rows": [
              ...
              ], title: `Choose One, Sis`
              })
          let po = client.prepareMessageFromContent(data.from, {
"listMessages":{
                  "title": `MENU J-BOT`,
                  "description": `${Time speech} sis *${data.pushname}*`,
                  "buttonText": "MENU!",
...
try {
if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send command *${data.prefix}anime [ query ]*\nExample : ${data.prefix}anime naruto`)
...
try {
if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send command *${data.prefix}manga [ query ]*\nExample : ${data.prefix}manga naruto`)
            data.reply(mess.wait)
...
try {
if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Send command *${data.prefix}chara [ query ]*\nExample : ${data.prefix}manga naruto`)
            data.reply(mess.wait)
...
*/
        Client.cmd.on('ytmp4', async(data) => {
            try {
                ...
                if(Number(ytm.size.split(' MB')[0]) >= 50.00) return Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', `*Data Successful Found!*\n\n*Title* : ${ytm.title}\n*Size* : ${ytm.size}\n*Quality* : ${ytm.quality}\n*Ext* : mp4\n *Link* : ${ytm.link}\n\n_For the duration of more than the limit presented in the form of link_`, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.thumb}`, 'thumb.jpg', text, data.message)
                Client.sendFileFromUrl(data.from, `${ytm.link}`, `${ytm.title} - Download.mp4`, `Video has been sent @${data.sender.split('@')[0] }`, data.message)
            } catch {
                data.reply('Oops sorry the server is down or maybe the apikey is invalid')
            }
        })
        Client.cmd.on('ytmp3', async(data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Send command *${data.prefix}ytmp3 [ link ]*\nExample : ${data.prefix}ytmp3 https://www.youtube.com/ watch?v=0maWbr0FHKY`)
                data.reply(mess.wait)
        ...
        Client.cmd.on('tiktok', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit)
               ...
               Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/socialmedia/tiktoknowm?url=${data.body}&apikey=BryanRfly`, 'tiktok.mp4', `Video Successfully Retrieved@${data .sender.split('@')[0]}`, data.message)
        })
        Client.cmd.on('playvid', async(data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Send command *${data.prefix}playvid [ query ]*\nExample : ${data.prefix}playvid amv`)
                data.reply(mess.wait)
        ...
        Client.cmd.on('play', async(data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Send command *${data.prefix}play [ link ]*\nExample : ${data.prefix}play alone`)
                data.reply(mess.wait)
        ...
        */
        ...
        Client.cmd.on('igstory', async(data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Send command *${data.prefix}igstory [ username ]*\nExample : ${data.prefix}igstory jessnolimit`)
                data.reply(mess.wait)
        ...
        Client.cmd.on('joox', async(data) => {
            try {
                if(isLimit(data.sender)) return data.reply(mess.limit)
                if(data.body == "") return data.reply(`Send command *${data.prefix}joox [ song ]*\nExample : ${data.prefix}joox bad wild`)
                data.reply(mess.wait)
                ...
                Client.sendFileFromUrl(data.from, `${jox.thumb}`, 'thumb.jpg', text, data.message)
                Client.sendFileFromUrl(data.from, `${jox.audio}`, 'audio.mp3', ``, data.message)
            } catch {
                data.reply('Sorry song not found')
            }
        })
        /*RANDOM*/
        Client.cmd.on('intake', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit)
            if(data.args[0].toLowerCase() == '1') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/intake/asupanukhty?apikey=${configs.uhykey}`, 'ukhty.mp4', `Here's Intake`, data.message)
            } else if(data.args[0].toLowerCase() == '2') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/intake/intake?apikey=${configs.uhykey}`, 'santuy.mp4', `Here Sis Asupan`, data.message)
            } else if(data.args[0].toLowerCase() == '3') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/intake/intake?apikey=${configs.uhykey}`, '+62.mp4',`Here Sis Intake`, data.message)
            } else if(data.args[0].toLowerCase() == '4') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/intake/asupanbocil?apikey=${configs.uhykey}`, 'bocil.mp4', `Here Sis Asupan`, data.message)
            } else if(data.args[0].toLowerCase() == '5') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/asupan/asupanrikagusriani?apikey=${configs.uhykey}`, 'rika.mp4', `Here Sis Asupan`, data.message)
            } else if(data.args[0].toLowerCase() == '6') {
              Client.sendFileFromUrl(data.from, `${configs.apiUhy}/api/intake/asupanghea?apikey=${configs.uhykey}`, 'ghea.mp4', `Here Sis Asupan`, data.message)
            } else if(data.args[0].toLowerCase() == '7') {
              Client.sendFileFromUrl(data.from, 'https://pencarikode.xyz/api/chika?apikey=pais', 'chika.mp4', `Here Sis Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == '8') {
              Client.sendFileFromUrl(data.from, 'https://pencarikode.xyz/api/intake?apikey=pais', 'random.mp4', 'Here', data.message)
            } else {
              let po = client.prepareMessageFromContent(data.from, {
"listMessages":{
                  "title": `INTRODUCTION MENU`,
                  "description": `Your intake menu is ${data.pushname}`,
                           ...
                           },
{
        ...
        Client.cmd.on('randomimg', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit)
            if(data.args[0].toLowerCase() == '1') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/cecan?apikey=${configs.lolkey}`, 'cecan.jpeg', 'Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '2') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/cogan?apikey=${configs.lolkey}`, 'cogan.jpeg', 'Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '3') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/bts?apikey=${configs.lolkey}`, 'bts.jpeg','Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '4') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/exo?apikey=${configs.lolkey}`, 'exo.jpeg', 'Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '5') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/aesthetic?apikey=${configs.lolkey}`, 'aesthetic.jpeg', 'Here is the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '6') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/blackpink?apikey=${configs.lolkey}`, 'bpink.jpeg', 'Here's Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '7') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/waifu?apikey=${configs.lolkey}`, 'waifu.jpeg', 'Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '8') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/husbu?apikey=${configs.lolkey}`, 'husbu.jpeg', 'Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '9') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/loli?apikey=${configs.lolkey}`, 'loli.jpeg', 'Here is the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '10') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/neko?apikey=${configs.lolkey}`, 'neko.jpeg','Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '11') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/elf?apikey=${configs.lolkey}`, 'elf.jpeg', 'Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '12') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/shota?apikey=${configs.lolkey}`, 'shota.jpeg', 'Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '13') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/sadgirl?apikey=${configs.lolkey}`, 'sadgirl.jpeg', 'Here's the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '14') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/art?apikey=${configs.lolkey}`, 'fanart.jpeg', 'Here is the Ramdom Image', data.message)
            } else if(data.args[0].toLowerCase() == '15') {
              Client.sendFileFromUrl(data.from, `${configs.apiLol}/api/random/wallnime?apikey=${configs.lolkey}`, 'wallnime.jpeg', 'Here's the Ramdom Image', data.message)
            } else {
              let po = client.prepareMessageFromContent(data.from, {
"listMessages":{
                  "title": "RANDOM IMAGE",
                  "description": `Menu Random Image Request From ${data.pushname}`,
                  "buttonText": "Click Here",
                  "listType": "SINGLE_SELECT",
                  "sections": [
                     {
                           ...
                           },
{
        ...
        Client.cmd.on('randomquran', async (data) => {
            ...
            text = `*Surah* : ${rquran.name}\n*Meaning* : ${rquran.meaning}\n*Verse* : ${rquran.asma}\n*Description* : ${rquran.description}`
            data.reply(text)
            Client.sendFileFromUrl(data.from, rquran.audio, 'quran.mp3', ``, data.message)
        })
        Client.cmd.on('aestheticpic', async (data) => {
            if(isLimit(data.sender)) return data.reply(mess.limit)
            Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/aestheticpic?apikey=${configs.zeksKey}`, 'aesthetic.jpg', ``, data.message)
        })
        ...
        Client.cmd.on('limit', async(data) => {
            ...
            if(dataUser[data.sender].premium) return data.reply(`Hi @${data.sender.split('@')[0]} \nYou are a premium user with unlimited access!` )
            limits = configs.maxLimit - dataUser[data.sender].limit
            if(limits <= 0) return data.reply("```" + `Your limit has expired` + "```")
            data.reply(`Hi @${data.sender.split('@')[0]} \n Your limit remains ${limits || 30}\nThe daily limit is reset at 00.00\nIf you want to get unlimited limit please chat owner bot type !owner`)
        })
        Client.cmd.on('sourcecode', async(data) => {
data.reply(`This bot was created in the Node.js / JavaScript programming language

Original source code bot : https://github.com/justpiple/whatsapp-bot

Source code used: https://github.com/Dvnz99/wabot

If an error occurs, you can contact the bot owner by typing ${data.prefix}owner`)
})
        /*OWNER*/
        ...
        Client.cmd.on('block', async(data) => {
            if(!data.isOwner) return data.reply(mess.ownerOnly)
            if(data.mentionedJidList.length == 0) return data.reply(`Send command *${data.prefix}${data.command} [ @tag ]*\nExample : ${data.prefix}${data. command} @0`)
        ...
        Client.cmd.on('addvn', async(data) => {
            ...
            if(data.body == "") return data.reply(`Send command ${data.prefix}addvn [ name ]\nExample ${data.command}addvn hi`)
            if(vn.includes(data.body)) return data.reply('Vn name already exists, please use another name')
        ...
        Client.cmd.on('delvn', async(data) => {
            ...
            data.reply(`Successfully removing vn ${data.body} from database`)
        })
        Client.cmd.on('listvn', async(data) => {
            let listvn = 'Type the vn name to download vn\n\n*List vn*:\n\n'
            vn.forEach((vnn, i) => listvn += `*${i+1}*. ${vnn}\n`)
            data.reply(listvn)
        })
        Client.cmd.on('guess the image', async (data) => {
			if(isLimit(data.sender)) return data.reply(mess.limit)
